...
# Preserve the selected method in local state even before the first wrap event.
...
# Recover wrap method from the cloud Wrapped event note.
...
old_restore='wrapped:recovered.wrapped,phase:finished?"Finished"'
new_restore='wrapped:recovered.wrapped,wrapMethod:recovered.wrapMethod||null,phase:finished?"Finished"'
if text.count(old_restore)!=1: raise SystemExit('restore anchor not found')
text=text.replace(old_restore,new_restore,1)
# Version labels are set by the newest extension near the end of index.html.
